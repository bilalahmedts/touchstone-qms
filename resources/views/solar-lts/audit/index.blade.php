...
@section('content')
    ...
    <div class="search-area pt-2 pb-2">
        ...
        <form action="{{ route('solar-lts.audit.index') }}" method="get" autocomplete="off">
            ...
            <div class="card card-primary card-outline mt-3" id="search" @if (!isset($_GET['search'])) style="display: none;" @endif>
                ...
                <div class="card-footer">
                    ...
                    <a href="{{ route('solar-lts.audit.index') }}" class="ml-5">Clear Search</a>
                </div>
            </div>
        </form>
    ...
    <div class="card card-primary card-outline">
        <div class="card-header">
            ...
            <div class="card-tools">
                <a href="{{ route('solar-lts.audit.create') }}" class="btn btn-primary btn-sm">
                    ...
                    @if (count($audits) > 0)
                        ...
                        @foreach ($audits as $audit)
                            <tr>
                                ...
                                <td>{{ $audit->user->supervisor->name ?? 'N/A' }}</td>
                                ...
                                <td>
                                    @if ((in_array(Auth::user()->roles[0]->name, ['Super Admin','Director','Manager', 'Team Lead'])))
                                    <a href="{{ route('solar-lts.audit.edit', $audit) }}" class="btn btn-primary btn-sm"><i
                                            class="fas fa-edit"></i></a>
                                    @endif
                                    @if ((in_array(Auth::user()->roles[0]->name, ['Super Admin'])))
                                    <a href="{{ route('solar-lts.audit.destroy', $audit) }}" class="btn btn-primary btn-sm"><i
