...
@section('title', 'Voice Audit Appeals')
...
<div class="search-area">
    ...
    <form action="{{ route('voice-audit-appeals.index') }}" method="get" autocomplete="off">
        ...
        <div class="card card-primary card-outline mt-3" id="search" @if(isset($_GET['search'])) style="display: block;" @endif>
            ...
            <div class="card-footer">
                ...
                <a href="{{ route('voice-audit-appeals.index') }}" class="ml-5">Clear Search</a>
            </div>
        </div>
    </form>
...
<div class="card card-primary card-outline">
    <div class="card-header">
        <h3 class="card-title">Voice Audit Appeals List</h3>
    </div>
                ...
                @if(count($voice_audits) > 0)
                    ...
                    @foreach ($voice_audits as $audit)
                        <tr>
                            ...
                            <td>
                                @if($audit->appeal->status == 'pending')
                                    <span class="badge bg-primary">Pending</span>
                                @elseif($audit->appeal->status == 'accepted')
                            ...
                            <td class="action">
                                <a href="{{ route('voice-audit-appeals.show', $audit) }}" class="btn btn-success btn-sm"><i class="fas fa-eye"></i></a>
                                @if(
                                    ...
                                    @if($audit->appeal->status == 0)
                                        <a href="{{ route('voice-audit-appeals.edit', $audit) }}" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i></a>
                                        <form action="{{ route('voice-audit-appeals.destroy', $audit) }}" method="post">
                                            @csrf
                                            @method('delete')
                                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure?')" style="padding: 0.25rem 10px;"><i class="fas fa-times"></i></button>
                                        </form>
                                    @endif
                                @endif
                            </td>
