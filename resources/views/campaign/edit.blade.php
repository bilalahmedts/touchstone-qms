    ...
    <form action="{{ route('campaigns.update', $campaign) }}" method="post" autocomplete="off">
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="name" value="{{ old('name', $campaign->name) }}" placeholder="Enter Campaign Name" required>
            </div>
            @error('name')
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="database_name" value="{{ old('database_name', $campaign->database_name) }}" placeholder="Enter Database Name">
            </div>
            @error('database_name')
                <div class="validate-error">{{ $message }}</div>
            @enderror
            ...
            <div class="form-group">
                ...
                <select name="status" class="form-control select2" required>
                    ...
                    <option value="disabled" @if($campaign->status == 'disabled') selected @endif>Disable</option>
                </select>
            </div>
        </div>
        <!-- /.card-body -->
